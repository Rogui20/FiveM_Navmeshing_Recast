...
int main(int argc, char** argv)
{
	...
	if (argc < 2) {
		std::cerr << "Uso: RecastDemo <MapName>" << std::endl;
		return 1;
	}

	std::string mapName = argv[1];
	PathRequestHandler pathHandler;
	pathHandler.mainLoopMonitor(mapName);

	return 0;
	
	//PathRequestHandler execute;
	//execute.ExecuteOperations(argc, argv);
