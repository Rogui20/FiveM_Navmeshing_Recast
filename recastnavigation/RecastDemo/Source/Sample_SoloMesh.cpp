...
void Sample_SoloMesh::applySettings(float agentRadius, float agentHeight, float cellSize, float AgentMaxClimb, float AgentMaxSlope)
{
...
void Sample_SoloMesh::setOffMeshConnectionsFromFile(const std::string& filePath)
{
    if (!m_geom) return;
    ...
    while (std::getline(file, line)) {
        ...
        float sx, sy, sz, ex, ey, ez, rad;
        int bidir, area, flags;

        if (ss >> sx >> sy >> sz >> ex >> ey >> ez >> rad >> bidir >> area >> flags) {
            float spos[3] = { sx, sy, sz };
            float epos[3] = { ex, ey, ez };
            m_geom->addOffMeshConnection(spos, epos, rad, (unsigned char)bidir, (unsigned char)area, (unsigned short)flags);
        }
    }
	file.close();
	fs::remove(filePath);
...
void Sample_SoloMesh::setOffMeshConnections(const std::vector<OffMeshConnectionStruct>& connections)
{
    if (!m_geom) return;
	...
	{
		const float* spos = conn.start;
		const float* epos = conn.end;

		float radius = conn.radius
		;
		unsigned char bidir = conn.bidirectional;
		unsigned char area = conn.area;
		unsigned short flags = conn.flags;

		m_geom->addOffMeshConnection(spos, epos, radius, bidir, area, flags);
	}
}
